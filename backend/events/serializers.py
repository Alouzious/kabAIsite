...
class EventListSerializer(serializers.ModelSerializer):
    """Enriched serializer for list views with all main detail fields for events"""
    category_name = serializers.CharField(source='category.name', read_only=True)
    image_thumbnail_url = serializers.SerializerMethodField()
    is_past = serializers.ReadOnlyField()
    is_upcoming = serializers.ReadOnlyField()
    ...
    class Meta: 
        ...
        fields = [
            ...
            'image_thumbnail_url', 'category', 'category_name',
            'date', 'time', 'end_date', 'location',
            'venue_details',
            'registration_link', 'registration_deadline', 'max_participants',
            'status', 'is_featured', 'is_past', 'is_upcoming'
        ]
